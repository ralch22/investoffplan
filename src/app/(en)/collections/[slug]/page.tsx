...
export default async function CollectionsSlugPage({
  ...
  const bySlug = t.bySlug as Record<
    string,
    { title: string; h1: string; description: string }
  >;
  const { slug } = await params;
  ...
  const unitCount = items.length.toLocaleString(fmtLocale);
  const projectCountLabel =
    seen.size >= MAX_PROJECTS ? String(MAX_PROJECTS) : String(projects.length);
  const heroSubtitle = interpolate(
    seen.size >= MAX_PROJECTS
      ? t.unitOptionsSubtitlePlus
      : t.unitOptionsSubtitle,
    { units: unitCount, projects: projectCountLabel },
  );
  const collectionPath = localePath(locale, `/collections/${page.slug}`);
  const jsonLd = {
        ...
        <section className="mt-12">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {otherCollections.map((other) => {
              const otherCopy = bySlug[other.slug] ?? {
                title: other.title,
                h1: other.h1,
