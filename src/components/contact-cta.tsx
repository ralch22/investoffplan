...
export function ContactCta({ locale = "en" }: { locale?: Locale }) {
  ...
  const INFO_ITEMS = [
    { label: f.brochure, icon: "📖" },
    { label: f.paymentPlans, icon: "📋" },
    { label: f.floorPlans, icon: "▦" },
    { label: f.availableUnits, icon: "⌂" },
  ];
      ...
      <div className="mx-auto grid grid-cols-1 max-w-[1200px] gap-10 px-5 md:grid-cols-2 md:items-center md:px-8">
        ...
        <div>
          <h2 className="font-display text-4xl font-semibold">
            {f.contactUs}<span className="text-brand">.</span>
          </h2>
          <p className="mt-3 text-white/80">
            {f.contactUsBody}
          </p>
          <ContactCtaForm locale={locale} />
        </div>
      </div>
    </section>
  );
