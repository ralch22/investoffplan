  ...
  if (s?.grossYieldPct != null && o?.grossYieldPct != null && s.grossYieldPct > o.grossYieldPct) {
    pros.push(
      interpolate(t.proHigherYield, {
        a: String(s.grossYieldPct),
        b: String(o.grossYieldPct),
      }),
    );
  }
  if (s?.medianPpsqft != null && o?.medianPpsqft != null && s.medianPpsqft < o.medianPpsqft) {
    pros.push(
      interpolate(t.proLowerPpsf, {
        a: s.medianPpsqft.toLocaleString(),
        b: o.medianPpsqft.toLocaleString(),
      }),
    );
  }
  if (
  ...
  if (s != null && o != null && s.saleSample > o.saleSample) {
    pros.push(
      interpolate(t.proDeeperResale, {
        a: s.saleSample.toLocaleString(),
        b: o.saleSample.toLocaleString(),
      }),
    );
  }
  if (side.area.projectCount > other.area.projectCount) {
    pros.push(
      interpolate(t.proMoreSupply, {
        projects: String(side.area.projectCount),
...
export function buildSuitability(
  ...
  if (extras.familyStockShare >= 0.3) {
    out.push({
      profile: t.suitFamiliesProfile,
      reason: interpolate(t.suitFamiliesReason, {
        pct: String(Math.round(extras.familyStockShare * 100)),
      }),
    });
  }
...
export function buildComparisonFaqs(
  ...
  if (a.stats?.grossYieldPct != null && b.stats?.grossYieldPct != null) {
    ...
    const lo = hi === a ? b : a;
    faqs.push({
      q: interpolate(t.faqYieldQ, { a: a.area.name, b: b.area.name }),
      a: interpolate(t.faqYieldA, {
        hi: hi.area.name,
        hiPct: String(hi.stats!.grossYieldPct),
        loPct: String(lo.stats!.grossYieldPct),
        lo: lo.area.name,
      }),
    });
  }
  if (a.stats?.medianPrice != null && b.stats?.medianPrice != null) {
    ...
    faqs.push({
      q: interpolate(t.faqCheaperQ, { a: a.area.name, b: b.area.name }),
      a: interpolate(t.faqCheaperA, {
        cheap: cheap.area.name,
        cheapPrice: money(cheap.stats!.medianPrice!),
        dearPrice: money(dear.stats!.medianPrice!),
        dear: dear.area.name,
        cheapFrom: money(cheap.area.minPriceAed),
        dearFrom: money(dear.area.minPriceAed),
      }),
    });
  }
  if (a.stats != null && b.stats != null) {
    ...
    faqs.push({
      q: interpolate(t.faqResellQ, { a: a.area.name, b: b.area.name }),
      a: interpolate(t.faqResellA, {
        liq: liq.area.name,
        liqSales: liq.stats!.saleSample.toLocaleString(),
        thinSales: thin.stats!.saleSample.toLocaleString(),
        thin: thin.area.name,
      }),
    });
  }
  ...
  faqs.push({
    q: t.faqSupplyQ,
    a: interpolate(t.faqSupplyA, {
      supply: supply.area.name,
      projects: String(supply.area.projectCount),
      units: supply.area.unitCount.toLocaleString(),
      scarceProjects: String(scarce.area.projectCount),
