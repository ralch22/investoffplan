...
export function buildComparisonFaqs(
  ...
  if (a.stats?.grossYieldPct != null && b.stats?.grossYieldPct != null) {
    ...
    const lo = hi === a ? b : a;
    faqs.push({
      q: interpolate(t.faqBetterYieldQ, { a: a.area.name, b: b.area.name }),
      a: interpolate(t.faqBetterYieldA, {
        hi: hi.area.name,
        hiYield: String(hi.stats!.grossYieldPct),
        loYield: String(lo.stats!.grossYieldPct),
        lo: lo.area.name,
      }),
    });
  }
  if (a.stats?.medianPrice != null && b.stats?.medianPrice != null) {
    ...
    faqs.push({
      ...
      a: interpolate(t.faqCheaperA, {
        ...
        cheapMedian: money(cheap.stats!.medianPrice!),
        dearMedian: money(dear.stats!.medianPrice!),
        dear: dear.area.name,
